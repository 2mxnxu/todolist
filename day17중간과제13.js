...
function 전체출력함수() {
    ...
    for(let index = 0; index<=할일목록.length-1; index++) {
        ...
        html += `<div class="contentBox ${info.할일상태 == true ? 'success' : ''}">
            ...
            <div class="contentBtns"><button onclick ="수정함수(${info.할일코드})" type = "button" class="updateBtn">수정</button>
            <button onclick="삭제함수(${info.할일코드})" type="button" class="deleteBtn">삭제</button>
            </div>
